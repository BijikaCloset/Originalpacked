...
class ProductPage extends React.PureComponent {
  ...
  componentDidMount() {
    console.log(this.props.location.state?.data, "i am data");
    this.setState({
      productData: this.props.location.state?.data,
    });

    const getBrandDetails = async () => {
      console.log("getting brand details");
      const response = await axios.post(`/api/brand/details`, {
        id: this.props.location.state?.data.brand,
      });
      console.log(response, "check me");
      if (response.data.success === true) {
        this.setState({
          brandDetails: response.data.brand[0],
        });
      }
    };
    getBrandDetails();
    ...
    const getProductImages = async () => {
      ...
      let res = await fetchProductImages({
        id: this.props.location.state?.data.imagesproducts,
      });
      console.log(res);
  ...
  render() {
    ...
    console.log(this.state.productData, "i am data");
                ...
                <div className="product-container">
                  <div className="item-box">
                    <div className="item-details">
                      ...
                      <p className="sku">{this.state.productData.sku}</p>
                      <hr />
                      {this.state.productData.brand && (
                        <p className="by">
                          see more from{" "}
                          <Link
                            to={`/shop/brand/${this.state.productData.brand.slug}`}
                            className="brand-link"
                          >
                            {this.state.productData.brand.name}
                          </Link>
                        </p>
                      )}
                      ...
                      <p className="price">${this.state.productData.price}</p>
                    </div>
                    <div className="item-customize">
                      <Input
                        ...
                        name={"quantity"}
                        min={1}
                        max={this.state.productData.inventory}
                    ...
                    {this.state.brandDetails?.name === "Bijika" ? (
                      <div className="item-customize">
                        <Input
                          type={"number"}
